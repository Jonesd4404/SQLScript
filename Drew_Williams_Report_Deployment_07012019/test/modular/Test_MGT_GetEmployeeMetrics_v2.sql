DECLARE @LocationNo VARCHAR(5) = '00007'
DECLARE @Employee_Login VARCHAR(20) = 'CJohnson2'
...
--2) All divisions implement ISNULL(n/NULLIF(d, 0), " ") logic. The end " ", which anywhere else is bad practice, 
...
SELECT
	...
	SUM(em.buys_count_TotalQty)/NULLIF(SUM(em.buys_count_BuyTrans), 0)		[buys_avg_EmpItemsPerBuy],
	...
	SUM(lm.buys_count_TotalQty)/NULLIF(SUM(lm.buys_count_BuyTrans), 0)		[buys_avg_LocItemsPerBuy],
	...
	CAST(SUM(em.buys_count_BuyTrans) AS FLOAT)/
		CAST(SUM(lm.buys_count_BuyTrans) AS FLOAT)													[buys_ttl_PctOfBuyTrans],

	CAST(SUM(em.buys_count_TotalQty) AS FLOAT)/
		CAST(SUM(lm.buys_count_TotalQty) AS FLOAT)													[buys_ttl_PctOfTotalQty],	
	...
	--	
	(SUM(em.buys_total_BuyScans)*SUM(lm.buys_count_BuyTrans))/
		NULLIF((SUM(lm.buys_total_BuyScans)*SUM(em.buys_count_BuyTrans)), 0) - 1					[buys_avg_PctDiffScansPerBuy],
	...
	SUM(em.buys_total_amtHB)/SUM(em.buys_total_qtyHB)	[buys_avg_EmpAmtHB],
	SUM(em.buys_total_qtyPB)							[buys_total_EmpQtyPB],
	SUM(em.buys_total_amtPB)/SUM(em.buys_total_qtyPB)	[buys_avg_EmpAmtPB],		
	SUM(em.buys_total_qtyDVD)							[buys_total_EmpQtyDVD],
	SUM(em.buys_total_amtDVD)/SUM(em.buys_total_qtyDVD) [buys_avg_EmpAmtDVD],	
	SUM(em.buys_total_qtyCD)							[buys_total_EmpQtyCD],
	SUM(em.buys_total_amtCD)/SUM(em.buys_total_qtyCD)	[buys_avg_EmpAmtCD],
	SUM(em.buys_total_qtyLP)							[buys_total_EmpQtyLP],
	SUM(em.buys_total_amtLP)/SUM(em.buys_total_qtyLP)	[buys_avg_EmpAmtLP],

	--Location Buy Item Metrics
	SUM(lm.buys_total_qtyHB)							[buys_total_LocQtyHB],
	SUM(lm.buys_total_amtHB)/SUM(lm.buys_total_qtyHB)	[buys_avg_LocAmtHB],
	SUM(lm.buys_total_qtyPB)							[buys_total_LocQtyPB],
	SUM(lm.buys_total_amtPB)/SUM(lm.buys_total_qtyPB)	[buys_avg_LocAmtPB],
	SUM(lm.buys_total_qtyDVD)							[buys_total_LocQtyDVD],
	SUM(lm.buys_total_amtDVD)/SUM(lm.buys_total_qtyDVD) [buys_avg_LocAmtDVD],
	SUM(lm.buys_total_qtyCD)							[buys_total_LocQtyCD],
	SUM(lm.buys_total_amtCD)/SUM(lm.buys_total_qtyCD)	[buys_avg_LocAmtCD],
	SUM(lm.buys_total_qtyLP)							[buys_total_LocQtyLP],
	SUM(lm.buys_total_amtLP)/SUM(lm.buys_total_qtyLP)	[buys_avg_LocAmtLP],
	...
	SUM(em.SIPS_count_amtNOST)/NULLIF(SUM(em.SIPS_count_qtyNOST), 0)	[SIPS_avg_EmpAmtNOST],
	...
	SUM(lm.SIPS_count_amtNOST)/NULLIF(SUM(lm.SIPS_count_qtyNOST), 0)	[SIPS_avg_LocAmtNOST],
	...
	(SUM(em.SIPS_count_amtNOST)*SUM(lm.SIPS_count_qtyNOST))/
		NULLIF((SUM(lm.SIPS_count_amtNOST)*SUM(em.SIPS_count_qtyNOST)), 0) - 1	[SIPS_avg_PctDiffAmtNOST],
