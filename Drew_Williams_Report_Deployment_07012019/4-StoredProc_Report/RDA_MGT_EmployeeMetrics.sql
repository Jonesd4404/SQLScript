...
-- =============================================
CREATE PROCEDURE [dbo].RDA_MGT_EmployeeMetrics
...
FROM Reports..RDA_RU_EmployeeMetrics 
WHERE 
...
SELECT
	...
	CAST(SUM(em.orders_count_SAS + em.orders_count_XFR) AS FLOAT)/
		CAST(SUM(lm.orders_count_SAS + lm.orders_count_XFR) AS FLOAT)		[orders_count_PctOfCombined],
	CAST(AVG(em.orders_count_SAS + em.orders_count_XFR) AS FLOAT)/
		CAST(AVG(lm.orders_count_SAS + lm.orders_count_XFR) AS FLOAT)		[orders_avg_PctOfMoQtyCombined], 
	CAST(SUM(em.orders_amt_SAS + em.orders_amt_XFR) AS FLOAT)/
		CAST(SUM(lm.orders_amt_SAS + lm.orders_amt_XFR) AS FLOAT)			[orders_total_PctOfAmtCombined]
FROM Reports..RDA_RU_EmployeeMetrics em
