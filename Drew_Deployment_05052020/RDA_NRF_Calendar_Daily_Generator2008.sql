...
BEGIN
	...
	SET @NRF_MonthName = CASE 
						WHEN @NRF_Week = 1 --Reset to month 1 on the first week of each year
							THEN DATENAME(MONTH, DATEADD(DAY, @di + 7, @StartWeekDate))
						WHEN @NRF_Week = 53
							THEN DATENAME(MONTH, DATEADD(DAY, @di, @StartWeekDate)) --To keep this in the same month as week 52, we have to go off the starting day of the week here.
						WHEN @NRF_Week % 13 = 1 
						    THEN DATENAME(MONTH, DATEADD(DAY, @di + 7, @StartWeekDate))
						WHEN @NRF_Week % 13 = 5 
							THEN DATENAME(MONTH, DATEADD(DAY, @di + 7, @StartWeekDate))
