...
SET @path = '\\C1-VEEAM\SQL_AGENT_BACKUPS\BCSBELL\'
...
END
 ...
 SET @FileName = @path + 'BCSBELL#' +  'TimeKeeper' + '#' +@FileYear + '-' + @FileMonth + '-' + @FileDay + 'T' + @FileHour + @FileMinute + @FileSecond + '#Full' + '.BAK'  
 BACKUP DATABASE [TimeKeeper] TO DISK = @FileName  WITH COMPRESSION, STATS = 1
 PRINT @FileName
...
--sqlNative_Gardner_Move_To_C1-VEEAM
-------------------------------------------------------------------
