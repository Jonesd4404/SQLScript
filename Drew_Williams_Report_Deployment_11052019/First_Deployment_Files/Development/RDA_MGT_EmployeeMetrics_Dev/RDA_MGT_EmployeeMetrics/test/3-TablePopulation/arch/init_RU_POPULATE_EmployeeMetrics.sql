---------------------------
--Title:	Employee Metrics Roll-up
--Author:	William Miller
--Date:		02/25/19
--Purpose:	Create a rollup of all metrics related to specific employee performance which may be needed for purposes of training or review
--
--Timeline:
--02/25/19  *Rough draft of queries including all metrics is created. Code is modularized and saved by module to allow for troubleshooting. 
--				This may result in eventual refactoring for purposes of optimization.
--03/07/19	*Exchanged all averages for totals and quantities to avoid taking averages of monthly averages in the report itself, which caused inaccuracies.
--03/08/19	*Added count of transaction voids and no-sales to register data
--
--Future	(Done) *Create a per location rollup to reference location totals and averages. This will be a separate query.
--			*Add number of items shelved per employee.	
--			*Modify #BuyItems to flag when an employee is responsible for more than X% (X value TBD) of a buy to make this the criteria for inclusion into the metrics 
--				report instead of end buyer.
--			*Modify #Buy Items and #RU_Buys to take all items entered by an employee, not just those entered where they are the end buyer.
--			*Add Top 3 sections by items shelved per employee, if possible. 
--				(Requires a massive join, sort, and rank given current state of data. A roll-up of first scans/last scans keyed to ShelfItemScanID may simplify this.)
--			*Once a preliminary version is finalized, create a query to update the table resulting from this query on a monthly basis.
--			**Once data becomes available from SAS cart, add SAS/XFR data to report.
--			**Once data becomes available from BackOffice roll-out, add till deviation data to report.
--
--Notes:
--02/25/19	*First run encompassed dates 01/01/2017 through 01/31/2019. Run time was roughly 25 minutes.
--03/08/19	*Run of updated version encompassed dates 01/01/2017 through 02/28/2018. Run time was approximately 28 minutes.
---------------------------
...
EXEC [Sandbox].[dbo].PARAMS_CreateDateRangeSelect 
...
INSERT INTO Sandbox..RU_EmployeeMetrics_test
SELECT 
	kt.LocationNo,
	kt.Employee_Login,
	kt.BusinessMonth,
