---------------------------
--Title:	Location Metrics Roll-up
--Author:	William Miller
--Date:		02/28/19
--Purpose:	Create a rollup of all metrics related to average location performance which may be needed for purposes of context in employee level reports
--
--Timeline:
--02/29/19  *Rough draft of queries including all metrics is created. Code is modularized and saved by module to allow for troubleshooting. 
--				This may result in eventual refactoring for purposes of optimization.
--03/08/19	*Exchanged all averages for totals and quantities to avoid taking averages of monthly averages in the report itself, which caused inaccuracies.
--03/08/19	*Added count of transaction voids and no-sales to register data
--
--Future	
--			*Add number of items shelved
--			*Once a preliminary version is finalized, create a query to update the table on a monthly basis.
...
--
--Notes:
--03/08/19	*Run of updated version encompassed dates 01/01/2017 through 02/28/2018. Run time was approximately 26 minutes.
---------------------------
...
EXEC [Reports].[dbo].PARAMS_CreateDateRangeSelect 
